...
public class RegistrationFacebook extends Activity {
	...
	private String fb_name,fb_gender,fb_email,fb_json,fb_firstname,fb_lastname,fb_userid,fb_flag;
	Typeface font_style;
	private Button join;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		fb_name=extras.getString("fb_name");
		fb_gender=extras.getString("fb_gender");
		fb_email=extras.getString("fb_email");
		fb_json=extras.getString("fb_json");
		fb_firstname=extras.getString("fb_firstname");
		fb_lastname=extras.getString("fb_lastname");
		fb_userid=extras.getString("fb_userid");
		fb_flag=extras.getString("fb_flag");
		context = getApplicationContext();
		initview();
		session = new SessionManager(getBaseContext()); 

		edit_firstname.setText(fb_firstname);
		edit_lastname.setText(fb_lastname);
		edit_email.setText(fb_email);
	...
	private class GetMemberGender extends AsyncTask<Void, Void, ArrayList<GenderModel>> 
	{
		...
		protected void onPreExecute() {
			...
			if(pDialog == null){
				pDialog = Util.createProgressDialog(RegistrationFacebook.this);
	...
	private void getUserDetails() {
		...
		{
			Toast.makeText(RegistrationFacebook.this,"Please select the gender",Toast.LENGTH_SHORT).show();
		}

		if(dob.length()<=0)
		{
			Toast.makeText(RegistrationFacebook.this,"Please select the DOB",Toast.LENGTH_SHORT).show();
		...
		if(firstname.length()>4 && (username.length()>=6) && (password.length()>=6) && (isEmailValid(cs)) && (spinner_gender.getSelectedItemPosition()>0)&& dob.length()>0)
		{
			if(terms_check.isChecked())
			{
				...
				ConnectionDetector conn = new ConnectionDetector(RegistrationFacebook.this);
				if(conn.isConnectingToInternet()){
					new Register().execute();	
				}else{
					Crouton.makeText(RegistrationFacebook.this, getString(R.string.crouton_message), Style.ALERT).show();
				}
			...
			{
				Toast.makeText(RegistrationFacebook.this, "Accept the terms and conditions", Toast.LENGTH_LONG).show();
			}
		}
		else
		{
			//Toast.makeText(RegistrationFacebook.this,"Enter valid details",Toast.LENGTH_LONG).show();
		}
	}
	...
	public class Register extends AsyncTask<Void, Void, JSONObject> {
		...
		protected JSONObject doInBackground(Void... params) {
			...
			String error = null;
			try {
				createRegisterJsonObject.put("oauth",fb_flag);
				createRegisterJsonObject.put("oauth_user_id",fb_userid);
				createRegisterJsonObject.put("firstname",fb_firstname);
				...
				createRegisterJsonObject.put("lastname",fb_lastname);
				createRegisterJsonObject.put("oauth_data",fb_json);
				...
				try {
					...
					if(responseMessage.equalsIgnoreCase("successfully registered"))
					{
						...
						session.createLoginSession(username, password,
								Util.USER_ID,Util.USEREMAIL,Util.API_TOKEN,Util.CHAT_PASSWORD,"Facebook","Facebook",fb_userid);
						// Navigation to  Nav Main to be implemented 
						...
						Toast.makeText(RegistrationFacebook.this, responseMessage,Toast.LENGTH_LONG).show();
						//clearDetails(); 
						...
						if(errorMessages.length()>0)
						{
							...
							Toast.makeText(RegistrationFacebook.this, error,Toast.LENGTH_LONG).show(); 	 
						}
					}
				} catch(JSONException e){}
			}
			...
			connection.addPacketListener(new PacketListener() {
				...
				public void processPacket(Packet packet) {
					...
					if (message.getBody() != null) {
						...
						DataBaseHandler datebasehandler = new DataBaseHandler(RegistrationFacebook.this);
							...
							if(checkConversations == null){
								...
								if(isApplicationSentToBackground(context)){
									NotificationCompat.Builder mBuilder =
											new NotificationCompat.Builder(RegistrationFacebook.this)
									.setSmallIcon(R.drawable.ic_launcher)
									.setContentTitle(checkConversations.getWith_user_display_name())
									.setContentText(1 + " new messages");
									// Creates an explicit intent for an Activity in your app
									Intent resultIntent = new Intent(RegistrationFacebook.this, ChatConversationsActivity.class);
									...
									TaskStackBuilder stackBuilder = TaskStackBuilder.create(RegistrationFacebook.this);
