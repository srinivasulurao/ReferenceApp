...
import com.youflik.youflik.register.RegistrationFacebook.Register;
...
public class RegistrationTwitter extends Activity {
	...
	private String tw_name,tw_screen,tw_location,tw_json,tw_id,tw_flag;
	Typeface font_style;
	private Button join;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		tw_screen=extras.getString("tw_screen");
		tw_location=extras.getString("tw_location");
		tw_json=extras.getString("tw_json");
		tw_id=extras.getString("tw_id");
		tw_flag=extras.getString("tw_flag");
		context = getApplicationContext();
		session = new SessionManager(getBaseContext()); 
		initview();
		edit_firstname.setText(tw_name);
		edit_username.setText(tw_screen);
	...
	private class GetMemberGender extends AsyncTask<Void, Void, ArrayList<GenderModel>> 
	{
		...
		protected void onPreExecute() {
			...
			if(pDialog == null){
				pDialog = Util.createProgressDialog(RegistrationTwitter.this);
	...
	private void getUserDetails() {
		...
		{
			Toast.makeText(RegistrationTwitter.this,"Please select the gender",Toast.LENGTH_SHORT).show();
		}

		if(dob.length()<=0)
		{
			Toast.makeText(RegistrationTwitter.this,"Please select the DOB",Toast.LENGTH_SHORT).show();
		...
		if(firstname.length()>4 && (username.length()>=6) && (password.length()>=6) && (isEmailValid(cs)) && (spinner_gender.getSelectedItemPosition()>0)&& dob.length()>0)
		{
			if(terms_check.isChecked())
			{
				...
				ConnectionDetector conn = new ConnectionDetector(RegistrationTwitter.this);
				if(conn.isConnectingToInternet()){
					new Register().execute();	
				}else{
					Crouton.makeText(RegistrationTwitter.this, getString(R.string.crouton_message), Style.ALERT).show();
				}
			...
			{
				Toast.makeText(RegistrationTwitter.this, "Accept the terms and conditions", Toast.LENGTH_LONG).show();
			}
		}
		else
		{
			//Toast.makeText(RegistrationTwitter.this,"Enter valid details",Toast.LENGTH_LONG).show();
		}
	}
	...
	public class Register extends AsyncTask<Void, Void, JSONObject> {
		...
		protected JSONObject doInBackground(Void... params) {
			...
			String error = null;
			try {
				createRegisterJsonObject.put("oauth",tw_flag);
				createRegisterJsonObject.put("oauth_user_id",tw_id);
				...
				createRegisterJsonObject.put("oauth_data",tw_json);
				...
				try {
					...
					if(responseMessage.equalsIgnoreCase("successfully registered"))
					{
						...
						session.createLoginSession(username, password,
								Util.USER_ID,Util.USEREMAIL,Util.API_TOKEN,Util.CHAT_PASSWORD,"Twitter","Twitter",tw_id);
						...
						Toast.makeText(RegistrationTwitter.this, responseMessage,Toast.LENGTH_LONG).show();
						//clearDetails(); 
						...
						if(errorMessages.length()>0)
						{
							...
							Toast.makeText(RegistrationTwitter.this, error,Toast.LENGTH_LONG).show(); 	 
						}
					}
				} catch(JSONException e){}
			}
			...
			connection.addPacketListener(new PacketListener() {
				...
				public void processPacket(Packet packet) {
					...
					if (message.getBody() != null) {
						...
						DataBaseHandler datebasehandler = new DataBaseHandler(RegistrationTwitter.this);
							...
							if(checkConversations == null){
								...
								if(isApplicationSentToBackground(context)){
									NotificationCompat.Builder mBuilder =
											new NotificationCompat.Builder(RegistrationTwitter.this)
									.setSmallIcon(R.drawable.ic_launcher)
									.setContentTitle(checkConversations.getWith_user_display_name())
									.setContentText(1 + " new messages");
									// Creates an explicit intent for an Activity in your app
									Intent resultIntent = new Intent(RegistrationTwitter.this, ChatConversationsActivity.class);
									...
									TaskStackBuilder stackBuilder = TaskStackBuilder.create(RegistrationTwitter.this);
