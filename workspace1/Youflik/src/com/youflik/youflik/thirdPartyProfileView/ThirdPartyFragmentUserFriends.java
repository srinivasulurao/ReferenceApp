...
import com.youflik.youflik.commonAdapters.ThirdPartyConnectionAdapter;
...
public class ThirdPartyFragmentUserFriends extends Fragment {
	...
	private ThirdPartyConnectionAdapter adapter;
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		pageCount = 0;
		adapter = new ThirdPartyConnectionAdapter(getActivity(), connectionssearch);
	...
	private class ConnectionsAsyncClass extends AsyncTask<Void, Void, ArrayList<FriendsModel>>{
		...
		protected ArrayList<FriendsModel> doInBackground(Void... params) {
			...
			JSONObject jsonObjectRecived = HttpGetClient.sendHttpPost(Util.API+"connection?user_id="+friedID);
		...
		protected void onPostExecute(ArrayList<FriendsModel> result) {
			...
			if(result == null){
				...
				NoConnections.setText(FriendName +" haven't connected to anyone yet!");
	...
	private class ConnectionsLoadMoreAsyncClass extends AsyncTask<Void, Void, ArrayList<FriendsModel>>{
		...
		protected ArrayList<FriendsModel> doInBackground(Void... params) {
			JSONArray connectionsResponse = null;
			JSONObject jsonObjectRecived = HttpGetClient.sendHttpPost(Util.API+"connection?user_id="+friedID +"&last_date=" + pagination_Date_String.replaceAll(" ", "%20"));
			if(jsonObjectRecived != null){
		...
		protected void onPostExecute(ArrayList<FriendsModel> result) {
			...
			if(result == null){
				...
				//adapter = new ThirdPartyConnectionAdapter(getActivity(), result);
