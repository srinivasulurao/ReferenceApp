...
public class AdapterCategory extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		holder.lblcount         = (TextView) convertView.findViewById(R.id.lblcount);
		
		HashMap<String, String> item = new HashMap<String, String>();
        item = data.get(position);
        
		holder.lblNameCategory.setText(item.get(userFunction.key_category_name));
		holder.lblcount.setText("("+item.get(userFunction.key_deal_count)+")");
		
		// Set marker with picasso
        Picasso.with(activity)
        .load(userFunction.URLAdmin+userFunction.folderAdmin+item.get(userFunction.key_category_marker))
        .fit().centerCrop()
        .tag(activity)
