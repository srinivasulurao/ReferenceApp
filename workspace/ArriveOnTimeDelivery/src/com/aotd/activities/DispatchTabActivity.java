...
public class DispatchTabActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if(Allorders.dispatchOrders != null)
		{
			marrDispatchList = Allorders.dispatchOrders;
		}
		
		
		marrDispatchListOffline = Allorders.dispatchOrders_;
		...
		acceptOrders.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				int size = tempArrSelectedOrders.size();
				if (tempArrSelectedOrders.size() > 0)
				{
					if (Utils.checkNetwork(getApplicationContext()))
					{
						...
						progressdialog = ProgressDialog.show(DispatchTabActivity.this,null,null);
					...
					else //OFFLINE:
					{
						...
						for(int i=0;i<tempArrSelectedOrders.size();i++)
						{
							OfflineDB db = new OfflineDB(DispatchTabActivity.this);
							DispatchAllListModel dm = new DispatchAllListModel();
							OfflineDataModel om = marrDispatchListOffline.get(i);
							dm.setOrder_id(om.getOrder_id());
							dm.setAccountName(om.getAccountName());
							dm.setAccountnotes("");
							
							dm.setCompany(om.getCompanyPU());
							dm.setAddress(om.getAddressPU());
							dm.setSuit(om.getSuitPU());
							dm.setCity(om.getCityPU());
							dm.setState(om.getStatePU());
							dm.setZip(om.getZipPU());
							dm.setPuHomephone(om.getHomePhonePU());
							dm.setPuCellPhone(om.getCellPhonePU());
							
							dm.setDlcompany(om.getCompanyDL());
							dm.setDladdress(om.getAddressDL());
							dm.setDlsuit(om.getSuitDL());
							dm.setDlcity(om.getCityDL());
							dm.setDlstate(om.getStateDL());
							dm.setDlzip(om.getZipDL());
							dm.setDlHomePhone(om.getHomePhoneDL());
							dm.setDlCellPhone(om.getCellPhoneDL());
						
							dm.setIsRoundTrip(om.getIsRoundtrip());
							dm.setHour(om.getHour());
							dm.setDLDate(om.getDLDate());
							dm.setRequestor(om.getRequestor());
							dm.setPeice(om.getPiece());
							dm.setWeight(om.getWeight());
							
							db.addData("Dispatch", dm);
							
						}
						
						Intent in = new Intent(DispatchTabActivity.this,PendingOrders.class);
						startActivity(in);
	...
	public void onResume() {
		...
		if(Utils.NetworkType(DispatchTabActivity.this).equalsIgnoreCase(Utils.wifi))
		{
			MainDispatchScreenTabView.imgOnline
			.setBackgroundResource(R.drawable.online);
			isClicked = true;
		}
		else if(Utils.NetworkType(DispatchTabActivity.this).equalsIgnoreCase(Utils.mobile))
		{
			MainDispatchScreenTabView.imgOnline
			.setBackgroundResource(R.drawable.online);
			isClicked = true;
	...
	public class DispatchAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			ArrayList<OfflineDataModel> aa = marrDispatchListOffline;
			String DLAdd = marrDispatchListOffline.get(position).getAddressDL();
			String DLCompany = marrDispatchListOffline.get(position).getCompanyDL();
			String PUAdd = marrDispatchListOffline.get(position).getAddressPU();
			String PUCompany = marrDispatchListOffline.get(position).getCompanyPU();
			...
			TextView pu_company = (TextView)v.findViewById(R.id.pu_company);
			TextView dl_company = (TextView)v.findViewById(R.id.dl_company);
			TextView dl_address = (TextView)v.findViewById(R.id.dl_address);
			TextView pu_address = (TextView)v.findViewById(R.id.pu_address);

			pu_company.setText("P/U : "+PUCompany );
			pu_address.setText("Address : "+PUAdd);
			dl_company.setText("D/L : "+DLCompany);
			dl_address.setText("Address: "+DLAdd);
			...
			mOrderStatus.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					if (mOrderStatus.getTag().equals("notclicked")) {
						mOrderStatus
						.setBackgroundResource(R.drawable.btn_dispatch_clicked);
						mOrderStatus.setTag("clicked");
						marrSelectedOrders.add(mOrderNum.getText().toString());
						int size = marrSelectedOrders.size();
						...
						mOrderStatus
						.setBackgroundResource(R.drawable.btn_dispatch_confirm);
						mOrderStatus.setTag("notclicked");
						for (int iCtr = 0; iCtr < marrSelectedOrders.size(); iCtr++) {
